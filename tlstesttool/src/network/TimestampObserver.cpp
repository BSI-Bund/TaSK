...
namespace TlsTestTool {
    ...
    public:
        ...
        Data(std::shared_ptr<TcpConnection> _tcpConnection, Tooling::Logger &_logger) : tcpConnection(_tcpConnection),
                                                                                        logger(_logger) {
        }
        ...
        static const timespec *getSwTimestamp(msghdr &msgHdr) {
            for (cmsghdr *cMsgHdr = CMSG_FIRSTHDR(&msgHdr);
                 nullptr != cMsgHdr; cMsgHdr = CMSG_NXTHDR(&msgHdr, cMsgHdr)) {
    ...
    TimestampObserver::TimestampObserver(std::shared_ptr<TcpConnection> connection, Tooling::Logger &logger)
        ...
        if (setsockopt(fd, SOL_SOCKET, SO_TIMESTAMPING, &flags, sizeof(flags)) < 0) {
            impl->log(__FILE__, __LINE__,
                      std::string{"setsockopt with SO_TIMESTAMPING failed: "} + std::strerror(errno));
        }
        ...
        if (getsockopt(fd, SOL_SOCKET, SO_TIMESTAMPING, &actualFlags, &len) < 0) {
            impl->log(__FILE__, __LINE__,
                      std::string{"getsockopt with SO_TIMESTAMPING failed: "} + std::strerror(errno));
