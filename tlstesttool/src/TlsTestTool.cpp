...
#include "network/StartTlsHandshake.h"
...
static bool configureTlsSession(const TlsTestTool::Configuration &configuration, TlsTestTool::TlsSession &tlsSession,
    ...
    tlsSession.setExtensionExtendedMasterSecret(configuration.isTlsExtendedMasterSecret());
    tlsSession.setPreSharedKey(configuration.getPreSharedKey(), configuration.getPskIdentity(),
                               configuration.getPskIdentityHint());
    if (configuration.getTlsLibrary() == TlsTestTool::Configuration::TlsLibrary::OPENSSL) {
        ...
        tlsSession.setServerHelloExtensions(configuration.getServerHelloExtension());
        tlsSession.setEncryptedExtensionsTls13(configuration.getEncryptedExtensionsTls13());
    }
    ...
    if (!configuration.getTlsSecretFile().empty()) {
        try {
            tlsSession.setSecretOutput(
                    std::make_unique<std::ofstream>(configuration.getTlsSecretFile(), std::ios::out | std::ios::app));
        } catch (const std::exception &e) {
    ...
    if (configuration.getTlsLibrary() == TlsTestTool::Configuration::TlsLibrary::OPENSSL) {
        ...
        if (configuration.getHandshakeType() ==
            TlsTestTool::Configuration::HandshakeType::SESSION_RESUMPTION_WITH_TICKET ||
            configuration.getHandshakeType() ==
            TlsTestTool::Configuration::HandshakeType::SESSION_RESUMPTION_WITH_SESSION_ID ||
...
static bool configureCertificates(const TlsTestTool::Configuration &configuration,
    ...
    if (!configuration.getCaCertificateFile().empty()) {
        try {
            tlsSession.setCaCertificate(configuration.getCaCertificateFile());
        } catch (const std::exception &e) {
...
static void waitForClosedTcpConnection(const TlsTestTool::Configuration &configuration,
                                       std::shared_ptr<TlsTestTool::TcpConnection> connection,
                                       Tooling::Logger &logger) {
                ...
                if (0 < connection->available()) {
                    ...
                    try {
                        ...
                        logger.log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                   ...
                                   Tooling::HexStringHelper::byteArrayToHexString(data));
...
static void setUpAndExecuteTlsSession(std::shared_ptr<TlsTestTool::TlsSession> tlsSession,
                                      TlsTestTool::Configuration &configuration,
                                      std::shared_ptr<TlsTestTool::TcpConnection> tcpConnection,
                                      Tooling::Logger &logger) {
...
static void
startServerAcceptHandler(std::shared_ptr<TlsTestTool::TlsSession> tlsSession, TlsTestTool::Configuration &configuration,
                         TlsTestTool::TcpServer &server, Tooling::Logger &logger, bool sessionResumption,
                         bool &serverHandledConnections) {
    server.getAcceptor().async_accept((server.getActiveTcpConnection()->getSocket()),
                                      [tlsSession, &configuration, &server, &logger, sessionResumption, &serverHandledConnections](
                                              std::error_code /*error*/) {
                                          logger.log(Tooling::LogLevel::HIGH, "Network", __FILE__, __LINE__,
                                                     "TCP/IP connection from " +
                                                     server.getActiveTcpConnection()->getRemoteIpAddress() + ':'
                                                     + std::to_string(
                                                             server.getActiveTcpConnection()->getRemoteTcpPort()) +
                                                     " received.");

                                          if (configuration.getStartTlsProtocol() !=
                                              TlsTestTool::Configuration::StartTLSProtocol::NONE) {
                                              bool result = TlsTestTool::StartTlsHandshake::executeStartTlsHandshake(
                                                      server.getActiveTcpConnection(), logger,
                                                      configuration.getStartTlsProtocol(), configuration, false);
                                              if (!result) {
                                                  logger.log(Tooling::LogLevel::HIGH, "Network", __FILE__, __LINE__,
                                                             "StartTLS pre-handshake failed");
                                                  return;
                                              }
                                          }

                                          setUpAndExecuteTlsSession(tlsSession, configuration,
                                                                    server.getActiveTcpConnection(), logger);
                                          std::cout.flush();
                                          if (sessionResumption) {
                                              ...
                                              std::cout.flush();
                                              startServerAcceptHandler(tlsSession, configuration, server, logger, false,
                                                                       serverHandledConnections);
                                          } else {
...
int main(int argc, char **argv) {
    ...
    if (TlsTestTool::Configuration::NetworkMode::CLIENT == configuration.getMode()) {
        ...
        if (configuration.getStartTlsProtocol() != TlsTestTool::Configuration::StartTLSProtocol::NONE) {
            bool result = TlsTestTool::StartTlsHandshake::executeStartTlsHandshake(client.getTcpConnection(), logger,
                                                                                   configuration.getStartTlsProtocol(),
                                                                                   configuration, true);
            if (!result) {
                logger.log(Tooling::LogLevel::HIGH, "Network", __FILE__, __LINE__, "StartTLS pre-handshake failed");
                return EXIT_SUCCESS;
            }
        }
        ...
        bool sessionResumption = configuration.getHandshakeType() ==
                                 TlsTestTool::Configuration::HandshakeType::SESSION_RESUMPTION_WITH_TICKET ||
                                 configuration.getHandshakeType() ==
                                 TlsTestTool::Configuration::HandshakeType::SESSION_RESUMPTION_WITH_SESSION_ID;
        ...
        startServerAcceptHandler(tlsSession, configuration, server, logger, sessionResumption,
                                 serverHandledConnections);
