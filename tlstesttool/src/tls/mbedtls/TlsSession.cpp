...
namespace TlsTestTool {
    namespace MbedTls {
        ...
        static void parseCertificate(std::istream &certificateInput, mbedtls_x509_crt *certificateOutput) {
            ...
            const auto result = mbedtls_x509_crt_parse(certificateOutput,
        ...
        static void parsePrivateKey(std::istream &privateKeyInput, mbedtls_pk_context *privateKeyOutput) {
            ...
            const std::string data{buffer.str()};
            const auto result = mbedtls_pk_parse_key(privateKeyOutput,
                                                     reinterpret_cast<const unsigned char *>(data.c_str()),
                                                     data.size(), nullptr, 0);
            ...
            Data(TlsSession &newTlsSession)
                ...
                assertSuccess("mbedtls_ssl_config_defaults",
                              mbedtls_ssl_config_defaults(&conf,
                                                          tlsSession.isClient() ? MBEDTLS_SSL_IS_CLIENT
            ...
            void tryToReadAlertMessage(bool forceRead) {
                ...
                while (isDataReadable()) {
                    ...
                    if (msgHeaderIndicatesAlert || forceRead) {
                        const auto readRecord = mbedtls_ssl_read_record(&ssl);
                        if (MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE == readRecord) {
                            tlsSession.log(__FILE__, __LINE__, "Fatal Alert message received.");
                            break;
                        } else if (MBEDTLS_ERR_SSL_INVALID_RECORD == readRecord) {
        ...
        void TlsSession::setCaCertificate(std::string caCertificatePath) {
            std::ifstream caCertificateFile(caCertificatePath);
            parseCertificate(caCertificateFile, &impl->caCertificate);
        ...
        void TlsSession::renegotiate() {
            ...
            if (impl->clientHelloExtensions.size() > 0) {
                ...
                impl->clientHelloExtensions.insert(impl->clientHelloExtensions.end(), renegInfoExt.begin(),
                                                   renegInfoExt.end());
                impl->clientHelloExtensions.insert(impl->clientHelloExtensions.end(), renegInfoExtSizes.begin(),
                                                   renegInfoExtSizes.end());
                impl->clientHelloExtensions.insert(impl->clientHelloExtensions.end(), impl->ssl.own_verify_data,
                                                   impl->ssl.own_verify_data + impl->ssl.verify_data_len);
        ...
        void TlsSession::setCipherSuites(const std::vector<TlsCipherSuite> &cipherSuites) {
            ...
            std::transform(cipherSuites.cbegin(), cipherSuites.cend(), impl->tlsCipherSuites.begin(),
                           [](const TlsCipherSuite &cipherSuite) {
                               return (cipherSuite.first << 8) | cipherSuite.second;
                           });
        ...
        void TlsSession::setEllipticCurveGroups(const std::vector<TlsEllipticCurveGroupID> &ellipticCurveGroups) {
            ...
            std::transform(ellipticCurveGroups.cbegin(), ellipticCurveGroups.cend(),
                           impl->tlsEllipticCurveGroups.begin(),
        ...
        void
        TlsSession::setSignatureAlgorithms(const std::vector<TlsSignatureAndHashAlgorithm> & /*signatureAlgorithms*/) {
            throw std::runtime_error("Function not supported with mbed TLS");
        }
        ...
        void TlsSession::setEncryptedExtensionsTls13(const std::vector<uint8_t> & /*encryptedExtensions*/) {
            throw std::runtime_error("setEncryptedExtensionsTls13 Function not supported with mbed TLS");
        ...
        void TlsSession::setExtensionExtendedMasterSecret(const bool enable) {
            mbedtls_ssl_conf_extended_master_secret(&impl->conf, enable ? MBEDTLS_SSL_EXTENDED_MS_ENABLED
