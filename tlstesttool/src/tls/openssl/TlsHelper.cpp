...
namespace TlsTestTool {
    namespace OpenSsl {
        ...
        const std::string TlsHelper::getInternalCipherSuite(const TlsCipherSuite cipherSuite) {
            if ((cipherSuite.first == 0x13) && (cipherSuite.second == 0x02)) {
                ...
                throw std::invalid_argument("Cipher suite \""
                        ...
                        {cipherSuite.first, cipherSuite.second}) + "\" not supported with OpenSSL");
            }
        }
        ...
        const std::string
        TlsHelper::getInternalSignatureAlgorithm(const TlsSignatureAndHashAlgorithm signatureAlgorithm) {
            ...
            switch (signatureAlgorithm.first) {
                ...
                default:
                    throw std::invalid_argument("Signature Algorithm (\""
                                                + std::to_string(signatureAlgorithm.first) + "," +
                                                std::to_string(signatureAlgorithm.second) +
        ...
        const std::string TlsHelper::getInternalSignatureScheme(const TlsSignatureScheme signatureScheme) {
            ...
            if (signatureScheme.first == 0x04 && signatureScheme.second == 0x01) {
                ...
                return "ecdsa_sha1";
            } else if (signatureScheme.first == 0x08 && signatureScheme.second == 0x1a) {
                return "ecdsa_brainpoolP256r1_sha256";
            } else if (signatureScheme.first == 0x08 && signatureScheme.second == 0x1b) {
                return "ecdsa_brainpoolP384r1_sha384";
            } else if (signatureScheme.first == 0x08 && signatureScheme.second == 0x1c) {
                return "ecdsa_brainpoolP512r1_sha512";
            } else {
                throw std::invalid_argument("Signature Scheme \""
                                            + Tooling::HexStringHelper::byteArrayToHexString(
                        {signatureScheme.first, signatureScheme.second}) + "\" not supported with OpenSSL");
            }
        }
