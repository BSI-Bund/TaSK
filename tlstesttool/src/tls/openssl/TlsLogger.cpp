...
#define CONTENT_TYPE_HEARTBEAT 24
        ...
        void
        TlsLogger::logInternalTls(Tooling::Logger *logger, int write_p, int version, int content_type, const void *buf,
                    ...
                    switch ((static_cast<const uint8_t *>(buf)[0])) {
                        case 0:
                            ...
                            if (len >= 3) {
                                ...
                                unsigned err = ((static_cast<const uint8_t *>(buf)[1]) << 8) +
                                               (static_cast<const unsigned char *>( buf)[2]);
                    ...
                    if (len > 0) {
                        switch ((static_cast<const uint8_t *>(buf)[0])) {
                            ...
                            case 8:
                                ...
                                break;
                            case 11: {
                                std::string clientCertPrefix = "";
                                if ((isClient && write_p) || (!isClient && !write_p)) {
                                    clientCertPrefix = "Client";
                                }
                                std::string finalString;
                                finalString = ", " + clientCertPrefix + "Certificate";
                                str_details1 = finalString.c_str();
                                break;
                            }
                            case 12:
        ...
        void TlsLogger::logTls(Tooling::Logger *logger, int write_p, int version, int content_type, const void *buf,
                               size_t len, bool isClient) {
                ...
                switch (content_type) {
                    case CONTENT_TYPE_CHANGE_CIPHER_SPEC:
                        logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                    (write_p ? TlsLogConstants::CHANGECIPHERSPEC_TX
                    ...
                    case CONTENT_TYPE_ALERT:
                        if (!write_p) {
                            ...
                            if (len == 2) {
                                logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                            Tooling::StringHelper::formatInt(TlsLogConstants::ALERT_LEVEL,
                                                                             static_cast<const uint8_t *>(buf)[0]));
                                logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                            Tooling::StringHelper::formatInt(TlsLogConstants::ALERT_DESCRIPTION,
                                                                             static_cast<const uint8_t *>(buf)[1]));
                            }
                        }
                        break;
                    case SSL3_RT_INNER_CONTENT_TYPE: //TLS 1.3
                        if (len == 1 && static_cast<const uint8_t *>(buf)[0] == CONTENT_TYPE_HEARTBEAT) {
                            logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                        (write_p ? TlsLogConstants::HEARTBEATMESSAGE_TRANSMITTED_TLS13
                                                 : TlsLogConstants::HEARTBEATMESSAGE_RECEIVED_TLS13));
                        }
                        break;
                    case CONTENT_TYPE_HEARTBEAT: //TLS 1.3
                        logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                    (write_p ? TlsLogConstants::HEARTBEATMESSAGE_TRANSMITTED_TLS13
                                             : TlsLogConstants::HEARTBEATMESSAGE_RECEIVED_TLS13));
                        break;
                    case CONTENT_TYPE_HANDSHAKE:
                        if (len > 0) {
                            ...
                            TlsMessageLogger::logTlsHandshakeMessage(*logger, handshakeType, write_p,
                                                                     std::vector<uint8_t>(data, data + (len - 4)),
                                                                     isClient);

                            switch (handshakeType) {
                                case TlsHandshakeType::HELLO_REQUEST:
                                    logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                                (write_p ? TlsLogConstants::HELLORETRY_TX
                                                         : TlsLogConstants::HELLORETRY_RX_VALID));
                                    break;
                                case TlsHandshakeType::CLIENT_HELLO:
                                    logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                                (write_p ? TlsLogConstants::CLIENTHELLO_TX
                                                         : TlsLogConstants::CLIENTHELLO_RX_VALID));
                                    break;
                                case TlsHandshakeType::SERVER_HELLO:
                                    logger->log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                                                (write_p ? TlsLogConstants::SERVERHELLO_TX
