    ...
    namespace OpenSsl {
        ...
        static void logServerHelloOrHelloRetryMessage(Tooling::Logger &logger, const std::vector<uint8_t> &msg) {
            ...
            */
            ...
            std::string helloRetryRequestRandomString(Tooling::HexStringHelper::byteArrayToHexString(
                    {helloRetryRequestRandom.cbegin(), helloRetryRequestRandom.cend()}));
        ...
        static void
        logCertificateMessageTls12(Tooling::Logger &logger, const std::vector<uint8_t> &msg, bool isClientCert) {
            std::string certificatePrefix = isClientCert ? "Client " : "";
            ...
            for (std::size_t certificateDataRead = 0; certificateDataRead < certificateListLength.get();) {
                ...
                logger.log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                           certificatePrefix + "Certificate.certificate_list[" + std::to_string(numCertificate) + "]="
            ...
            logger.log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                       certificatePrefix + "Certificate.certificate_list.size=" + std::to_string(numCertificate));
        }

        static void
        logCertificateMessageTls13(Tooling::Logger &logger, const std::vector<uint8_t> &msg, bool isClientCert) {
            auto ptr = msg.cbegin();
            uint8_t certType = *(ptr);
            std::string certificatePrefix = isClientCert ? "Client " : "";

            logger.log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                       certificatePrefix + "Certificate.CertificateType=" + std::to_string(certType));
            ptr += sizeof(uint8_t);
        ...
        static void logCertificateMessage(Tooling::Logger &logger, const std::vector<uint8_t> &msg, bool isClientCert) {
            ...
            logger.log(Tooling::LogLevel::HIGH, "TLS", __FILE__, __LINE__,
                       certificatePrefix + "Certificate message data="
            ...
            if (std::distance(ptrCertListTls12, msg.cend()) == certificateListLengthTls12.get()) {
                return logCertificateMessageTls12(logger, msg, isClientCert);
            } else if (std::distance(ptrCertListTls13, msg.cend()) == certificateListLengthTls13.get()) {
                return logCertificateMessageTls13(logger, msg, isClientCert);
            }
        }
        ...
        void TlsMessageLogger::logTlsHandshakeMessage(Tooling::Logger &logger, const TlsHandshakeType &type,
            ...
            if (TlsHandshakeType::SERVER_HELLO == type) {
                ...
                logCertificateMessage(logger, msg, (isSent && isClient) || (!isSent && !isClient));
