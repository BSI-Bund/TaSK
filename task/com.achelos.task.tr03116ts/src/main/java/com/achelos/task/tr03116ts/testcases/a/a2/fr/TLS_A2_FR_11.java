...
/**
 * Test case TLS_A2_FR_11 - Session resumption with Session Ticket.
 * <p>
 * Positive test verifying the session resumption through the Session Ticket.
 */
public class TLS_A2_FR_11 extends AbstractTestCase {

	private static final String TEST_CASE_ID = "TLS_A2_FR_11";
	...
	protected final void executeUsecase() throws Exception {
		...
		tfServerSessionClose.executeSteps("9", "Close TLS connection successfully", Arrays.asList(), testTool);

		testTool.saveInitialHandshakeLogs();

		tFDutClientNewConnection.executeSteps("10",
		...
		if(tlsVersion == TlsVersion.TLS_V1_2) {
			step(11, "Check if the TLS ClientHello correctly initiates session resumption via Session Ticket.",
			...
			step(11, "Check if the TLS ClientHello correctly initiates session resumption via PSK.",
		...
		step(12, "Check if the session resumption is executed without errors and the channel is established.",
		...
		tfApplicationCheck.executeSteps("13", "", Arrays.asList(), testTool, dutExecutor);

		tfLocalServerClose.executeSteps("14", "Server closed successfully", Arrays.asList(),
				testTool);
