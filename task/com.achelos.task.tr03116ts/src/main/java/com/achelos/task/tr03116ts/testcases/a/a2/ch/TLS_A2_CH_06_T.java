...
/**
 * Test case TLS_A2_CH_06_T - Server cert based on wrong domain parameters.
 * 
 * <p>
 ...
 * 
 */
public class TLS_A2_CH_06_T extends AbstractTestCase {

	private static final String TEST_CASE_ID = "TLS_A2_CH_06_T";
	 ...
	 * <h3>Description</h3>
	 * <ol>
	 * <li>The server supplies a certificate chain [CERT_SHORT_KEY] with a key length not conform to the application requirements.
			...
			if (tlsVersion == TlsVersion.TLS_V1_2) {
				...
				for (HashMap.Entry<String, List<TlsCipherSuite>> certificateTypeCipherSuite : certificateTypeCipherSuites
					...
					tfServerHello.executeSteps("3",
							...
							Arrays.asList(), testTool, tlsVersion, cipherSuites, new RequestClientCertificate());
			...
			if (tlsVersion == TlsVersion.TLS_V1_3) {
				...
				for (var signatureScheme : signatureSchemeListTls13) {
					...
					tfServerHello.executeSteps("3",
							"The TLS server answers the DUT choosing a TLS version and a cipher suite that is "
									+ "contained in the ClientHello.",
							Arrays.asList(), testTool, tlsVersion, signatureScheme, tls13CipherSuites, TlsTestToolTlsLibrary.OpenSSL, new RequestClientCertificate());
