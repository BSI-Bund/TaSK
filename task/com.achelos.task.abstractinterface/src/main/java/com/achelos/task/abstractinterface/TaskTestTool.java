...
public class TaskTestTool {
	...
	private void executeTaskTestTool(final File micsFile, final List<File> certificateFileList,
			final boolean ignoreMicsVerification, final boolean noExecution,  final String reportDirectory, final String clientAuthCertChainFile,
			final String clientAuthKeyFile, final String certificateValidationRootCA) {
		...
		// skip run plan execution and finish, if "no-execution" flag is set
		if (noExecution) {
			logger.info("TaSK: Skipping run plan execution ('no-execution' == true).");
		} else {
			executeTaskTestToolRun(testRunPlanFile, reportDirectory, false, clientAuthCertChainFile, clientAuthKeyFile, certificateValidationRootCA);
		}
	}

    private void executeTaskTestTool(final File testRunPlanFile, final String reportDirectory, final String clientAuthCertChainFile, final String clientAuthKeyFile, final String certificateValidationRootCA) {
        logger.warning(
                "TaSK: test run plan file is provided. Skipping the MICS file verification and running provided test run plan.");
        executeTaskTestToolRun(testRunPlanFile, reportDirectory, true, clientAuthCertChainFile, clientAuthKeyFile, certificateValidationRootCA);
    }
	...
	*/
    ...
    private void executeTaskTestToolRun(final File testRunPlanFile, final String reportDirectory,
                                        final boolean doLogDutInformation, final String clientAuthCertChainFile, final String clientAuthKeyFile, final String certificateValidationRootCA) {
        // Start of Phase II: Execute the test cases in the TRP
        ...
        try {
            ...
            testRunPlanConfig = TestRunPlanConfiguration.parseRunPlanConfiguration(testRunPlanFile, globalConfiguration,
                    reportDirectory, clientAuthCertChainFile, clientAuthKeyFile, certificateValidationRootCA);
		...
		if (testRunPlanConfig.getDutCallCommandGenerator() != null) {
			testRunPlanConfig.getDutCallCommandGenerator().finalizeRMI();
		}
    }
